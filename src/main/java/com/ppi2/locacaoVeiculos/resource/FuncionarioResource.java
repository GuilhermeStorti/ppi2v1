...
import com.ppi2.locacaoVeiculos.Service.FuncionarioService;
import com.ppi2.locacaoVeiculos.domain.Funcionario;
import com.ppi2.locacaoVeiculos.domain.Locacao;
import com.ppi2.locacaoVeiculos.representation.FuncionarioRepresentation;
...
public class FuncionarioResource {
	...
	public @ResponseBody HttpEntity<List<FuncionarioRepresentation>> listAll() {
		...
		funcionarios.forEach(funcionario -> {
			representations.add(new FuncionarioRepresentation(funcionario));
		});
	...
	public @ResponseBody HttpEntity<Void> update(@RequestBody FuncionarioRepresentation representation, @PathVariable("id") Long id) {
	...
	/*
	 * Funcionário -> Locações Cad
	 */
	
	@RequestMapping(value = "/{id}/locacoes_cad", method = RequestMethod.GET, produces = "application/json")
	public @ResponseBody HttpEntity<List<LocacaoRepresentation>> findLocacaoByIdFuncionarioCad(@PathVariable("id") Long id) {
				
		Funcionario funcionario = funcionarioService.findById(id);
		List<Locacao> locacoes = funcionario.getLocacoesCad();
	...
	/*
	 * Funcionário -> Locações Rec
	 */
	
	@RequestMapping(value = "/{id}/locacoes_rec", method = RequestMethod.GET, produces = "application/json")
	public @ResponseBody HttpEntity<List<LocacaoRepresentation>> findLocacaoByIdFuncionarioRec(@PathVariable("id") Long id) {
				
		Funcionario funcionario = funcionarioService.findById(id);
		List<Locacao> locacoes = funcionario.getLocacoesRec();
		List<LocacaoRepresentation> representations = new ArrayList<>();
		locacoes.forEach(locacao -> {
			representations.add(new LocacaoRepresentation(locacao));
		});
